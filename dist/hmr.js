document.addEventListener("error", (event) => {
    console.error(event);
    console.log("[sserver] Error occurred");
});
...
ws.addEventListener("open", async () => {
    console.log("Connected to sserver");
});
ws.addEventListener("error", async () => {
    console.log("[sserver] Error");
});
...
function getAndUpdateStyle(event) {
    if (!event.path)
        return;
    const el = Array.from(document.querySelectorAll("link")).find((e) => event.path && !oldLinkEls.has(e) && cleanUrl(e.href).includes(event.path));
    if (!el) {
        return;
    }
...
function getAndUpdateScript(event) {
    ...
    changes.forEach((path) => {
        if (!window.moduleSrcStore.includes(path))
...
function handleWarnings(event) {
    console.warn(`[sserver] Error: can't properly monitor changes in the following paths ${JSON.stringify(event.paths)}, changes in this files will cause full-reload`);
}
