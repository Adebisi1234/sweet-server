...
const mode = options.mode && typeof options.mode !== "boolean"
    ? options.mode
    : config.get("mode") ?? "hmr";
// Watching the current working directory
const watcher = chokidar.watch(process.cwd(), {
    ignored: (path) => path.includes(".git") || path.includes("node_modules"), // only watch js files
    persistent: true,
    ignoreInitial: true,
});
// Gzip
    ...
    try {
        ...
        if (fs.statSync(filePath).isDirectory()) {
            ...
            if (fileList.find((x) => x.name.includes("index.html"))) {
                ...
                return res.send(generateCustomIndexHtml(fileList));
            }
        }
        ...
        if (contents.includes("<head>")) {
            contents = root.toString().replace("<head>", `<head>
      <!-- Injected by sserver -->
      <script>
      ...
      `);
            ...
            contents = contents.replace("</body>", `<!-- Injected by sserver -->
      <script>
        window.__sserverPort = ${port}
        window.moduleSrcStore = ${JSON.stringify(moduleSrcStore)}
      </script>
    ...
    try {
        ...
        if (mode === "hmr") {
            if (pathname.endsWith(".css")) {
                console.log(chalk.yellow(`changes detected in `, `${chalk.bold(pathname)} reloading styles`));
                ...
                console.log(chalk.yellow(`changes detected in `, `${chalk.bold(pathname)} reloading module`));
                const paths = await invalidatedModuleList(pathname);
...
function generateCustomIndexHtml(fileList) {
    fileList = fileList.filter((fileOrDir) => !(fileOrDir.name.includes("node_modules") ||
...
function recursiveHtmlList(fileOrDir, id) {
    if (!fileOrDir.isDirectory()) {
        return `<a href="./${path.relative(process.cwd(), path.join(fileOrDir.parentPath, fileOrDir.name))}">
